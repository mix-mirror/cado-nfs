...
case "$JOB_NAME" in
    *"merge coverage tests"*)
        ...
        # ... and gcc, which we want for gcov and gcov-tool rather than
        # for compiling anything. gcov-tool does the counter merge, and
        # gcov is what grcov shells out to. Note that this job therefore
        # does need a gcc of the same version as the one that built the
        # objects in the "coverage tests N/M" jobs: both run on
        # alpine:latest, so they agree.
        needs_gcov=1
    ;;
esac
...
after_package_install() {
    ...
    if [ "$needs_gcov" ] ; then
        echo " + needs_gcov is set"
        debian_packages="$debian_packages     gcc"
        alpine_packages="$alpine_packages     gcc"
    fi
