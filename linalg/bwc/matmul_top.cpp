...
static int matmul_top_init_fill_balancing_header(matmul_top_data & mmt, matmul_top_matrix & Mloc, param_list_ptr pl)
{
    ...
    if (pi->m->jrank == 0 && pi->m->trank == 0) {
        ...
        } else if (access(Mloc.bname.c_str(), R_OK) == 0) {
 ...
 */
static void matmul_top_check_balancing_versus_parallelizing_info(matmul_top_data & mmt)
{
    ...
    for(size_t i = 0 ; i < mmt.matrices.size() ; i++) {
        ...
        int lok = 1;
        lok = lok && mmt.pi->wr[0 ^ (i & 1)]->totalsize == Mloc.bal.nv;
        lok = lok && mmt.pi->wr[1 ^ (i & 1)]->totalsize == Mloc.bal.nh;
        ok  = ok && lok;

        if (!lok && mmt.pi->m->jrank == 0 && mmt.pi->m->trank == 0) {
            fprintf(stderr, "Matrix %zu, %s: balancing file %s"
                    " has dimensions %ux%u,"
                    " this conflicts with the current run,"
                    " which expects dimensions (%ux%u)x(%ux%u).\n",
                    i,
                    Mloc.mname.c_str(), Mloc.bname.c_str(),
...
static void matmul_top_fill_balancing_for_all_matrices(matmul_top_data & mmt, param_list_ptr pl)
{
    size_t nok = 0;
    for(auto & Mloc : mmt.matrices) {
        nok += matmul_top_init_fill_balancing_header(mmt, Mloc, pl);
    }
    ASSERT_ALWAYS(nok == 0 || nok == mmt.matrices.size());

    if (nok == 0) {
        if (mmt.pi->m->jrank == 0 && mmt.pi->m->trank == 0) {
            std::vector<mf_bal_args> mbas(mmt.matrices.size());
            for(size_t i = 0 ; i < mmt.matrices.size() ; i++) {
                matmul_top_matrix & Mloc = mmt.matrices[i];
                printf("Creating balancing file %s\n", Mloc.bname.c_str());
                mf_bal_args & mba = mbas[i];
                mba.rwfile = NULL;
                mba.cwfile = NULL;
                ...
                mf_bal_adjust_from_option_string(&mba,
                        param_list_lookup_string(pl, "balancing_options"));
                /* since withcoeffs is a switch for param_list, it is
            ...
            mf_bal_n(mbas.data(), mmt.matrices.size());
        }
    ...
    for(size_t i = 0 ; i < mmt.matrices.size() ; i++) {
        ...
        Mloc.locfile = matrix_get_derived_cache_filename_stem(Mloc.mname, i & 1, mmt.pi, Mloc.bal.checksum);
    }
...
static void matmul_top_read_submatrix(matmul_top_data & mmt, int midx, param_list_ptr pl, int optimized_direction)
{
    ...
    if (!cache_loaded) {
        ...
        if (Mloc.mname.empty()) {
            ...
            if (ssm) {
                std::string submat = matrix_get_derived_submatrix_filename(Mloc.mname, midx & 1, mmt.pi);
