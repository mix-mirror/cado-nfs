...
void mf_bal_n(struct mf_bal_args * mbas, size_t n)
{
    const char * text[2] = { "row", "column", };
    int rc;
    int rectangular = mbas[0].rectangular;

    char *(*freeit)[2] = (char * (*) [2]) malloc(n * sizeof(char *[2]));
    std::vector<balancing> bals(n);
    ...
    for(size_t midx = 0 ; midx < n ; midx++) {
        ...
        bals[midx].nh = mbas[midx].nh;
        bals[midx].nv = mbas[midx].nv;
    }
    /* }}} */
    ...
    struct per_dim_stats {
        ...
        uint32_t (*perm)[2] = { NULL, };
        int todo = 0;
    };

    struct per_matrix_stats {
        uint32_t * weights[2] = { NULL, };
        uint32_t nzero[2] = { 0, 0 };
        uint32_t totalweight = 0;
        double avg[2] = { 0, 0 };
        double sdev[2] = { 0, 0 };
    };

    std::vector<per_dim_stats> D(n+1);
    std::vector<per_matrix_stats> M(n);
    ...
    for(size_t midx = 0 ; midx < n ; midx++) {
        ...
        D[midx].matsize = bals[midx].nrows;
        ASSERT_ALWAYS(!midx || bals[midx-1].ncols == bals[midx].nrows);
    }
    D[n].matsize = bals[n-1].ncols;
    /* }}} */

    /* {{{ define block sizes and padding for both inner and outer dimensions */
    for(size_t didx = 0 ; didx < n + 1 ; didx++) {
         ...
         */
        for ( ; D[didx].blocksize % MINIMUM_ITEMS_IN_BWC_CHUNKS ; D[didx].blocksize++);
    }
    ...
    for(size_t didx = 0 ; didx < n + 1 ; didx++) {
        ...
        printf("Padding dimension %zu to %u+%u=%u , which is %u blocks of %u\n",
     ...
     * We also fill bal.{ncoeffs, nzrows, nzcols, flags} for each matrix
     */
    for(size_t midx = 0 ; midx < n ; midx++) {
        balancing & bal = bals[midx];
        ...
        for(int d = 0 ; d < 2 ; d++) {
            ...
            M[midx].weights[d] = (uint32_t *) malloc(nitems * sizeof(uint32_t));
        ...
        bal.nzrows = M[midx].nzero[0];
        bal.nzcols = M[midx].nzero[1];
        bal.flags = 0;
    }
    /* }}} */
    ...
    for(size_t didx = 0 ; didx < n + 1 ; didx++) {
        ...
        if (dp[0] == mf_bal_args::MF_BAL_PERM_YES && dp[1] == mf_bal_args::MF_BAL_PERM_NO) {
            /* ok */
        } else if (dp[0] == mf_bal_args::MF_BAL_PERM_NO && dp[1] == mf_bal_args::MF_BAL_PERM_YES) {
            /* ok */
        } else if (dp[0] == mf_bal_args::MF_BAL_PERM_NO && dp[1] == mf_bal_args::MF_BAL_PERM_NO) {
            /* ok. No work to do here ! */
        } else if (dp[0] == mf_bal_args::MF_BAL_PERM_AUTO && dp[1] == mf_bal_args::MF_BAL_PERM_AUTO) {
            int choose = M[mi[1]].sdev[1] > M[mi[0]].sdev[0];
            printf("Choosing a %s (matrix %zu) permutation based"
                    " on largest deviation"
                    " (%.2f > %.2f)\n",
                    text[choose], mi[choose],
                    M[mi[choose]].sdev[choose],
                    M[mi[!choose]].sdev[!choose]);
            mbas[mi[choose]].do_perm[choose] = mf_bal_args::MF_BAL_PERM_YES;
            mbas[mi[!choose]].do_perm[!choose] = mf_bal_args::MF_BAL_PERM_NO;
        } else {
            ...
            fprintf(stderr,
                    ...
                    " at dimension %zu ; doperm[%zu][1]=%d, doperm[%zu][0]=%d\n",
                    didx,
    ...
    for(size_t didx = 0 ; didx < n + 1 ; didx++) {
        ...
        for(int d = 0 ; d < 2 ; d++)  {
            int midx = (didx + n - d) % n;
            if (mbas[midx].do_perm[d] == mf_bal_args::MF_BAL_PERM_YES) {
        ...
        D[didx].perm = (uint32_t (*)[2]) malloc(nitems * sizeof(uint32_t[2]));
    ...
    for(size_t didx = 0 ; didx < n + 1 ; didx++) {
        size_t nitems = D[didx].matsize + D[didx].padding;
        for(int d = 0 ; d < 2 ; d++)  {
            size_t midx = (didx + n - d) % n;

            if (mbas[midx].do_perm[d] == mf_bal_args::MF_BAL_PERM_NO)
            ...
            *pperm = (uint32_t *) malloc(nitems * sizeof(uint32_t));
