...
/* the 64-bit word number i of an mpz */
inline uint64_t word64(mpz_srcptr z, int i)
{
    uint64_t r = 0;
    for (int j = 0; j * GMP_LIMB_BITS < 64; j++)
        r |= uint64_t(mpz_getlimbn(z, i * (64 / GMP_LIMB_BITS) + j)) << (j * GMP_LIMB_BITS);
    return r;
}
...
int dispatch(mpz_srcptr N, Method && method, SetF && set_f)
{
    ...
    if (bits <= 64)
        return run.template operator()<arithxx_modredc64>(
                Integer64(word64(N, 0)));
    if (bits <= 96)
        return run.template operator()<arithxx_modredc96>(
                Integer128(word64(N, 0), word64(N, 1)));
    if (bits <= 126)
