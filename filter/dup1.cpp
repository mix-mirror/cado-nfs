...
struct set_of_files {
    ...
    void write_line(const char * p, const char * q)
    {
        ...
        file->put('\n');
        if (!file->good())
            throw std::runtime_error(fmt::format("Error writing relation to {}", filename));
        lines_left--;
    }
};// }}}
...
struct dup1_process {
    ...
    void filter3(filter_t const & F, std::vector<std::string> const & files)
    {
    ...
    template<typename ab_type, int base>
    void filter2(std::vector<std::string> const & files)
    {
        using relation_type = dup1_relation<ab_type, base>;
        if (force_posix) {
            using L = cado::filter_io_details::ifb_locking_posix;
            filter3(filter_rels<L, relation_type>(), files);
        } else {
    ...
    void filter1(std::vector<std::string> const & files)
    {
        if (abhexa) {
            filter2<ab_type, 16>(files);
        } else {
            filter2<ab_type, 10>(files);
        }
    }
    ...
    {
        if (largeab) {
            filter1<cxx_mpz>(files);
        } else {
            filter1<uint64_t>(files);
        }
        for (unsigned int i = 0; i < nslices; i++) {
