...
bool check_whether_file_is_renumbered(std::string const & filename)
{
    ...
    if (!f_tmp)
        throw std::runtime_error(fmt::format("{}: {}", filename, strerror(errno)));
    ...
    for(;;) {
        ...
        if (ret == NULL) {
            ...
            throw std::runtime_error(fmt::format("Error while reading {}", filename));
        }
        if (strlen(s) >= 1023)
            throw std::runtime_error(fmt::format("Too long line while reading {}", filename));
        size_t i = 0;
        while (s[i] == ' ')
            i++;
        if (s[i] != '#')
    ...
    if (count == 1)
        ...
        throw std::runtime_error(fmt::format("Invalid line in {}"
...
struct dup2_process { /* {{{ */
    ...
    void read()
    {
        ...
        if (!cpoly.read(polyfilename))
            throw std::runtime_error(fmt::format("cannot read {}", polyfilename)); // E: Use of undeclared identifier 'polyfilename'
    ...
    void filter_old_2(filter_t const & F, std::vector<std::string> const & files)
    {
        using relation_type = filter_t::relation_type;
        F(files, nullptr, nullptr,
                [this](relation_type & rel) { hash_renumbered_relation(rel); });
    }

    template<typename ab_type> void filter_old_1(std::vector<std::string> const & files) /* {{{ */
    {
        ...
        if (force_posix) {
            ...
            filter_old_2(filter_rels<L, relation_type>(), files);
        } else {
    ...
    void filter_already_renumbered_rels(std::vector<std::string> const & files) /* {{{ */
    {
        ...
        if (largeab)
            filter_old_1<cxx_mpz>(files);
        else
            filter_old_1<uint64_t>(files);
    ...
    template<typename filter_t>
    void filter_new_2(filter_t const & F, std::vector<std::string> const & files)
    {
        using relation_type = filter_t::relation_type;
        ...
        for(auto const & f : files) {
            ...
            if (rc < 0)
                throw std::runtime_error(fmt::format("rename({} -> {}): {}",
    ...
    template<typename ab_type> void filter_new_1(std::vector<std::string> const & files) /* {{{ */
    {
        /* TODO: tnfs and such! */
        ...
        /* because we have a complicated processing in filter_new_2, we
         * must have posix locking (currently)
         */
        using L = cado::filter_io_details::ifb_locking_posix;
        filter_new_2(filter_rels<L, relation_type>(), files);
    } /* }}} */
    ...
    void filter_new_rels(std::vector<std::string> const & files) /* {{{ */
    {
        ...
        if (largeab)
            filter_new_1<cxx_mpz>(files);
        else
            filter_new_1<uint64_t>(files);
    } /* }}} */
