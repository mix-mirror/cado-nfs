 ...
 * Must be called outside of a parallel region.
 * May move rows in memory and update row pointers (in rows).
 */
void heap_garbage_collection(heapctx_t ctx, typerow_t **rows);

/*
 * Deallocate all rows.
 * Does not release memory (it remains available for row allocation)
 */
