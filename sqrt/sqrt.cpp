...
calculateGcd (std::string const & prefix, unsigned int numdep, cxx_mpz const & Np)
{
    ...
    for (int side = 0; side < 2; ++side) {
        ...
        f >> sidesqrt[side];
        if (!f)
            throw std::runtime_error(fmt::format("cannot read side-{} sqrt from file {}", side, sidename));
    }
...
struct task_prepare_dependencies /* {{{ */
{
    ...
    void prepare_abs()
    {
        ...
        /* Check that kername has consistent size */
        {
            ...
            if (stat(kername.c_str(), sbuf) < 0)
                throw std::runtime_error(fmt::format("stat({}): {}",
    ...
    template <typename locking_layer, typename ab_type>
    void filter1()
    {
        using relation_type = cado::relation_building_blocks::ab_block<ab_type, 16>;
        using L = locking_layer;
        using F = filter_rels<L, relation_type>;
        std::vector<std::string> files {purgedname};
        F()(files, nullptr, nullptr, [&](auto & rel) { sqrt(rel); });
    }
    ...
    {
        if (largeab)
            filter1<locking_layer, cxx_mpz>();
        else
            filter1<locking_layer, uint64_t>();
    }
    ...
    void create_dependencies(std::string const & prefix)
    {
        ...
        if (filter_rels_force_posix_threads)
            filter1<cado::filter_io_details::ifb_locking_posix>();
        else
            filter1<cado::filter_io_details::ifb_locking_lightweight>();
