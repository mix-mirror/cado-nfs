...
void sm_side_info::compute_piecewise(cxx_mpz_poly & dst, cxx_mpz_poly const & u) const
{
    ...
    for(size_t j = 0; j < pieces.size() ; j++) {
        ...
        if (SM->deg == -1) {
            ...
            int v = valuation_of_ideal_at_prime_ideal(M, I, pieces[j].a, 1, ell);
            ...
            */
            ...
            for(int w = 0 ; w < v ; w++) {
                uO = multiply_elements_in_order(M, uO, pieces[j].a);
                mpz_mat_divexact_mpz(uO, uO, ell);
                fmt::print("{{" "({})*(a/ell)^{} is {}/{} relative to O" "}}",
                        u, w+1,
                        write_order_element_as_vector(uO), duO);
            }
...

{
    ...
    O = p_maximal_order(f0, ell);
    M = multiplication_table_of_order(O, f0);
    for(auto const & Ie : factorization_of_prime(O, f0, ell, cxx_gmp_randstate())) {
        cxx_mpz_mat const& I(Ie.first);
        // int e = Ie.second;
        cxx_mpz_mat a = valuation_helper_for_ideal(M, I, ell);
        auto two = prime_ideal_two_element(O, f0, M, I);
        ...
        std::string uniformizer = write_element_as_polynomial(theta_q, "alpha");
