...
void sm_side_info::compute_piecewise(cxx_mpz_poly & dst, cxx_mpz_poly const & u) const
{
    ...
    for(size_t j = 0; j < pieces.size() ; j++) {
        ...
        mpz_poly_pow_mod_f_mod_mpz (SM, u,
                pieces[j].g,
                pieces[j].exponent,
                ell2);
        ...
        if (SM->deg == -1) {
            ...
            int v = numbertheory_internals::valuation_of_ideal_at_prime_ideal(M, I, pieces[j].a, 1, ell);
#ifdef WIP_SM
            ...
            */
...
#ifdef WIP_SM
                fmt::print("{{" "killer is {}/{}" "}}", kill.print_poly("alpha"), dtheta);
#endif
            }
...
#ifdef WIP_SM
            /* multiply by a^v */
            fmt::print("{{" "{} is {}/{} relative to O" "}}",
                    u, numbertheory_internals::write_order_element_as_vector(uO), duO);
#endif
            for(int w = 0 ; w < v ; w++) {
                uO = numbertheory_internals::multiply_elements_in_order(M, uO, pieces[j].a);
                mpz_mat_divexact_mpz(uO, uO, ell);
#ifdef WIP_SM
                fmt::print("{{" "({})*(a/ell)^{} is {}/{} relative to O" "}}",
                        u, w+1,
                        numbertheory_internals::write_order_element_as_vector(uO), duO);
#endif
            }
            ...
            mpz_poly_pow_mod_f_mod_mpz (SM, u_fixed_poly,
                    pieces[j].g,
                    pieces[j].exponent,
                    ell2);
#ifdef WIP_SM
...

{
    ...
    for(auto const & Ie : numbertheory_internals::factorization_of_prime(O, f0, ell, cxx_gmp_randstate())) {
        cxx_mpz_mat const& I(Ie.first);
        // int e = Ie.second;
        cxx_mpz_mat a = numbertheory_internals::valuation_helper_for_ideal(M, I, ell);
        ...
        std::string uniformizer = numbertheory_internals::write_element_as_polynomial(theta_q, "alpha");
        ...
        for(auto & pp : pieces) {
            ...
            auto I = numbertheory_internals::generate_ideal(O, M, gens);
            int v = numbertheory_internals::valuation_of_ideal_at_prime_ideal(M, I, a, 1, ell);
            if (v == 0)
                continue;
