...
#pragma GCC diagnostic pop
    ...
    void no_dash_in_input_files(std::vector<std::string> const & input_files);
...
/*}}}*/
    ...
    void parse_outer_helper(relation_type & rel, uint64_t num, iterator_type it)
    requires requires { rel.parse_outer(it); }
    {
        rel.parse_outer(it);
        rel.num = num;
    }
    template<typename iterator_type, typename ... variants>
    void parse_outer_helper(std::variant<variants...> & rel, uint64_t num, iterator_type it)
    {
        rel = {};
...
struct filter_rels {
            ...
            Functions&& ...FF) const
    {
        ...
        std::thread producer(
                cado::filter_io_details::filter_rels_producer_thread,
                std::ref(rb),
                prepare_grouped_command_lines(input_files),
                tstats);
            ...
            for(size_t avail_offset = 0; avail_offset < avail_seen && (nl = rb.strchr('\n', 0)) > 0 ; ) {
                if (*rb.begin() != '#') {
                    ...
                    if (!active || bit_vector_getbit(active, relnum)) {
                        ...
                        ASSERT_ALWAYS(slot);
                        cado::filter_io_details::parse_outer_helper(*slot, relnum, rb.begin());
